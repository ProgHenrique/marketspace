...
import { AuthContext, PreAnnouncement } from "@/contexts/auth-context";
import { useRouter } from "next/router";
import { parseCookies } from "nookies";
import { GetServerSideProps } from "next";
import { Announcement } from "../index.page";
...
export default function UpdateAnnouncement({announcement}: UpdateAnnouncementProps) {
  const [files, setFiles] = useState<string[]>([])
  const [isImagesError, setIsImagesError] = useState(false)
  const [price, setPrice] = useState('')
  const radioValue = announcement.title ? announcement.isNew ? 'new': 'used': undefined

  const router = useRouter()

  // add images on files array
  useEffect(()=> {
    if(announcement.images.length > 0) {
      setFiles(announcement.images)
    }
  },[announcement.images])

  // remove error of no images on submit
  useEffect(()=> {
    if(files.length > 0) {
  ...
  // add value of existent announcement to edit
  useEffect(()=> {
    setValue('title', announcement.title)
    setValue('description', announcement.description)
    if(announcement.price > 0) {
      const newPrice = priceFormatted(String(announcement.price))
      setPrice(newPrice)
  ...
  },[announcement.description, announcement.price, announcement.title, price, radioValue, setValue])
  ...
  // update announcement object on database
  async function updateAnnouncement ({condition,description,price,title}: CreateAnnouncementFormData){
    await api.put('announcements/update/announcement', {
      id: announcement.id,
      description,
      title,
      images: files,
    ...
    const redirectUrl = '/' + router.asPath.split('/').slice(1, -1).join('/')
  ...
  return (
    <>
      <Head>
        <title>Editar anúncio | Marketspace</title>
      </Head>
      <Container>
        <Header>
        ...
        <Main>
          ...
          <section>
            ...
            <Form as='form' onSubmit={handleSubmit(updateAnnouncement)}>
              <Label>
